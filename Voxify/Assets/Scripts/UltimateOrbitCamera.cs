	...
	void Update() {
		if (target != null) {
			...
			if (mouseControl) {
				...
				*/
				if (!clickToRotate || VoxelEditor.Instance.OrbitKey) {
					xVelocity += Input.GetAxis(mouseAxisX) * xSpeed * invertXValue;
					yVelocity -= Input.GetAxis(mouseAxisY) * ySpeed * invertYValue;

					spinning = false;
				}
				if (!VoxelEditor.Instance.CursorDistanceMod) zoomVelocity -= Input.GetAxis(mouseAxisZoom) * zoomSpeed * invertZoomValue;
			}
			if (keyboardControl) {
